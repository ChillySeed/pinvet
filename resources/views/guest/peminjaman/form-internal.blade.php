...
@section('title', 'Form Peminjaman Internal - PINVET')
        ...
        <div class="col-md-8">
            <div class="card shadow">
                <div class="card-body p-5">
                    <h2 class="text-center mb-4">Form Peminjaman Internal</h2>
                    <p class="text-center text-muted mb-4">Isi data Anda sebagai anggota UKM</p>
                    ...
                    <form action="{{ route('peminjaman.store') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="nim_peminjam" class="form-control @error('nim_peminjam') is-invalid @enderror" 
                                       value="{{ old('nim_peminjam') }}" required>
                                @error('nim_peminjam')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Jabatan di UKM</label>
                                <input type="text" name="jabatan_peminjam" class="form-control @error('jabatan_peminjam') is-invalid @enderror" 
                                       value="{{ old('jabatan_peminjam') }}" required>
                                @error('jabatan_peminjam')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">UKM/Organisasi</label>
                        ...
                        <button type="submit" class="btn btn-primary w-100">Lanjut ke Preview Surat</button>
                    </form>
                </div>
            </div>
        </div>
